...
void SingleEndedOutput::updateUI(QLabel *labels[], QLineEdit *values[])
{
    ...
    auto styleForHeadroomSource = [&]() -> QString {
    ...
    // Inputs: rows 0..3
    const int inputParams[4] = { SE_VB, SE_VS, SE_IA, SE_RA };
    for (int r = 0; r < 4; ++r) {
        const int p = inputParams[r];
        if (parameter[p] && labels[r] && values[r]) {
            labels[r]->setText(parameter[p]->getName());
            values[r]->setText(QString::number(parameter[p]->getValue(), 'f', 1));
    ...
    const RowOut outs[] = {
        { 4, SE_VK,   "Bias point Vk (V):", 3 },
        { 5, SE_IK,   "Cathode current (mA):", 3 },
        { 6, SE_RK,   "Cathode resistor (\u03a9):", 3 },
        { 7, SE_POUT, "Max output power (W):", 2 },
        { 8, SE_PHEAD,"Power at headroom (W):", 2 },
    };
    for (const RowOut &o : outs) {
        if (labels[o.row] && values[o.row] && parameter[o.param]) {
            ...
            if (o.param == SE_PHEAD) {
                const QString style = styleForHeadroomSource();
    ...
    // THD: row 9
    if (labels[9] && values[9] && parameter[SE_THD]) {
        labels[9]->setText("THD at headroom (%):");
        if (!device1) {
            values[9]->setText("N/A");
        } else {
            values[9]->setText(QString::number(parameter[SE_THD]->getValue(), 'f', 2));
        }
        const QString style = styleForHeadroomSource();
    ...
    if (labels[12] && values[12] && parameter[SE_HEADROOM]) {
        labels[12]->setText("Headroom (Vpk):");
        values[12]->setText(QString::number(parameter[SE_HEADROOM]->getValue(), 'f', 1));
        const QString style = styleForHeadroomSource();
    ...
    if (labels[sensIndex] && values[sensIndex]) {
        ...
        if (device1) {
            ...
            const double ia_mA    = parameter[SE_IA]->getValue();
            ...
            if (effectiveHeadroomVpk > 0.0 && raa > 0.0) {
                ...
                // Evaluate gm at the actual DC anode voltage implied by the
                // chosen bias current and load, rather than at Va = Vb. This
                // keeps the small-signal gain used for sensitivity aligned
                // with the operating point used for headroom and THD.
                double vaBias = vb;
                if (raa > 0.0 && ia_mA > 0.0) {
                ...
                double iaPlus  = device1->anodeCurrent(vaBias, vgBias + dVg, vs);
                double iaMinus = device1->anodeCurrent(vaBias, vgBias - dVg, vs);
    ...
    if (labels[14] && values[14] && labels[15] && values[15] && device1 && effectiveHeadroomVpk > 0.0) {
        ...
        values[14]->setText(QString("%1 / %2")
                                .arg(parameter[SE_HD2]->getValue(), 0, 'f', 1)
                                .arg(parameter[SE_HD4]->getValue(), 0, 'f', 1));
        ...
        values[15]->setText(QString::number(parameter[SE_HD3]->getValue(), 'f', 1));
    ...
    // Remaining parameter slots are intentionally left untouched here so
    // other circuits (or future rows) can reuse them without being hidden.
}
                                                    ...
                                                    double &thd) const
{
    ...
    // Require a valid operating point and positive headroom.
    if (vb <= 0.0 || raa <= 0.0 || iaBias_mA <= 0.0 || headroomVpk <= 0.0) {
        return false;
    }

    // Approximate small-signal gain Av around the current bias so that we can
    ...
    const double dVg = std::max(0.05, std::abs(vgBias) * 0.02); // at least 50 mV

    double iaPlus  = device1->anodeCurrent(vaBias, vgBias + dVg, vs);
    double iaMinus = device1->anodeCurrent(vaBias, vgBias - dVg, vs);
    double gm_mA_per_V = 0.0;
    if (std::isfinite(iaPlus) && std::isfinite(iaMinus) && dVg > 0.0) {
        // anodeCurrent returns mA, so the central difference directly yields gm in mA/V.
        gm_mA_per_V = (iaPlus - iaMinus) / (2.0 * dVg);
    }
    ...
    double gain = std::abs(gm_A_per_V * raa); // bypassed-cathode small-signal gain
    if (gainMode == 0 && rk > 0.0) {
        // Unbypassed cathode: include local feedback from Rk.
        const double feedback = 1.0 + gm_A_per_V * rk;
        if (feedback > 1.0 && std::isfinite(feedback)) {
            gain /= feedback;
        }
    ...
    const double Vpp_out_target = 2.0 * headroomVpk;      // desired anode Vpp
    double Vpp_in = Vpp_out_target / gain;                // initial grid Vpp estimate
