...
{
    // Input parameters mirroring web PushPull defaults
...
void PushPullOutput::update(int index)
{
    ...
    const double ia  = parameter[PP_IA]->getValue(); // mA per tube
    ...
    for (int i = 0; i < 101; ++i) {
        ...
        // Smooth max (approximate smax) as in JS
        const double k = 5.0;
    ...
    if (device1->getDeviceType() == PENTODE) {
        // Device::screenCurrent already returns mA, matching the
        // Model::screenCurrent convention. Do not rescale by 1000.
        ik_mA += device1->screenCurrent(vb, -bestVg1, vs);
    }
