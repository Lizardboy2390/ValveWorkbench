...
- [x] 2025-12-10: Designer **Triode Common Cathode** Headroom Waveshape viewer wired to render a DC-removed, peak-normalised anode-voltage waveform Va(t) over one cycle at the effective headroom (manual or helper-derived), updating whenever Triode CC recomputes headroom/THD, Max Sym Swing, or K-bypass.
- [x] 2025-12-10: Designer **Triode Common Cathode** circuit wired with a Headroom (Vpk) parameter and time-domain HD2/HD3/HD4/HD5/THD-at-headroom metrics reusing the SE-style helper semantics; cir13 now drives `TRI_CC_HEADROOM`, effective headroom follows manual vs symmetric/max swing helpers with matching colour cues, and the shared Max Sym Swing / K-bypass checkboxes have been moved to a dedicated row directly beneath the HD2/HD4 + HD3/HD5 headroom helper display in the Designer parameter column and are only shown for circuits that use these stage toggles (Triode CC, Pentode CC, SE/SE-UL/PP/UL-PP outputs; hidden for null/other circuits).
